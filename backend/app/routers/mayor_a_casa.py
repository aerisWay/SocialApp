...
# ── Helper: generar PDF ────────────────────────────────────────
def _build_pdf(casos: list) -> bytes:
    """Genera un PDF con la lista de casos activos usando ReportLab."""
    ...
    story = [
        Paragraph("Mayor a Casa — Informe de Casos Activos", title_st),
        Paragraph(
    ...
    # Cabeceras
    headers = [
        "Apellidos", "Nombre", "DNI / SIP", "Zona",
        "Teléfono", "Mes Renov.", "F. Alta", "Dirección",
    ]
    col_widths = [4*cm, 3.5*cm, 2.8*cm, 1.5*cm, 3*cm, 2.5*cm, 2.5*cm, None]
...
@router.get("/casos/informe/pdf", summary="Generar informe PDF de casos activos")
